...
- 職責：
  ...
  4. 呼叫資料存取層進行資料操作，提供資料存取的抽象介面給資料存取層 (六邊形架構)
  5. 呼叫領域模型進行業務邏輯處理(充血領域模型) / 實現業務規則與政策（貧血領域模型）
  6. 回傳輸出解果
...
- 類型：
  ...
  3. Business Rules/Policy (商業規則): 定義業務規則與政策，「無狀態」的業務邏輯實現
     - 職責：從 Service 抽離業務規則（三層是架構）、提供跨 Aggregates 操作（DDD 架構）
     - 用於抽離特定業務邏輯，例如：優惠金額計算器、信用卡結賬日計算器、特定業務規則驗證器等。
