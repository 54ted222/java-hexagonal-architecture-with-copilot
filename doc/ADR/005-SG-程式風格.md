...
- 儘量設計 API 與方法為幂等 (Idempotent)，避免重複執行導致資料不一致
- 幂等操作的定義：無論執行多少次，結果都是一樣的

### 純粹函式 (Pure Function)

- 儘量設計純粹函式，避免副作用 (Side Effects)
- 純粹函式的定義：相同輸入永遠產生相同輸出，且不修改外部狀態

### 無依賴/無狀態類別

- 儘量設計無依賴/無狀態類別 (Stateless Class)，避免共享狀態導致的競爭條件
- 無依賴類別的定義：類別不依賴於任何外部狀態或變數

- 儘量設計不可變類別 (Immutable Class)，避免物件狀態被修改
- 不可變類別的定義：物件一旦建立後，其狀態不可被改變
